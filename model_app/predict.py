...
num_labels= 7
xlmr_tokenizer = AutoTokenizer.from_pretrained("./tokenizer/")
...
def predict_nertags(text):
    ...
    for ele in prompt_list:
        if ele not in skill_nertags:
            list_of_tags.append("")
        else:
            list_of_tags.append(skill_nertags[ele])
    return list_of_tags

# ner_res = predict_nertags("मोदी बांगलादेश दौऱ्यावर जाणार आहेत आणि जर्मनी")
# for i,val in enumerate(ner_res):
#     print(val[0],val[1])
#     # print(val,ner_res[1][i].pop())